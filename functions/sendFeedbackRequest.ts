...
Deno.serve(async (req) => {
  try {
    ...
    const { booking_id, is_scheduled_run } = await req.json();

    // If scheduled run, use service role; otherwise verify admin access
    if (!is_scheduled_run) {
    ...
    // If no booking_id, process all eligible bookings (scheduled run)
    ...
    if (booking_id) {
      ...
      // Get all approved/completed bookings
      const allBookings = await base44.asServiceRole.entities.RoomBooking.list();
      const now = new Date();
      
      // Filter bookings that have ended and need feedback
      bookingsToProcess = allBookings.filter(booking => {
        ...
        const timeSinceEnd = (now - bookingDateTime) / (1000 * 60); // minutes
        
        return timeSinceEnd >= FEEDBACK_DELAY_MINUTES;
      });
    }
    ...
    for (const booking of bookingsToProcess) {
      ...
      const feedbackLink = `${Deno.env.get('BASE44_APP_URL') || 'https://dark-claim-flow-pro.base44.app'}/RoomFeedback?bookingId=${booking.id}`;
      ...
      const emailPromises = allAttendees.map(async (attendee) => {
        ...
        const alreadySubmitted = existingFeedbacks.some(f => f.attendee_id === attendee.email);
        ...
        try {
          ...
          await base44.asServiceRole.entities.Notification.create({
            ...
            title: 'Action Required: Share Your Feedback',
            message: `Your meeting in ${booking.room_name} is completed. Please share your feedback.`,
            is_read: false,
            email_sent: true
          });
          ...
          return { email: attendee.email, status: 'failed', error: error.message };
        }
      });

      const attendeeResults = await Promise.all(emailPromises);
    ...
    return Response.json({
      success: true,
      processed_bookings: results.length,
      results
    });
